...
export function normalizeTask(value: unknown): Task | null {
  ...
  return {
    ...
    priority: t.priority,
    label: t.label,
    severity: (t as any).severity ?? "minor",
    reviewer: (t as any).reviewer ?? "",
    storyPoints: Number.isFinite((t as any).storyPoints) ? (t as any).storyPoints : 0,
    estimatedEffort: Number.isFinite((t as any).estimatedEffort) ? (t as any).estimatedEffort : 0,
    dependencies: Array.isArray((t as any).dependencies) ? (t as any).dependencies : [],
    subtasks: Array.isArray((t as any).subtasks) ? (t as any).subtasks : [],
    checklist: Array.isArray((t as any).checklist) ? (t as any).checklist : [],
    recurrence: (t as any).recurrence ?? { frequency: "none" as const, interval: 1, endsOn: "" },
